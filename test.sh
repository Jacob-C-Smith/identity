...
if [ -n "$(sudo docker ps -aq --filter name=identity_)" ]; then
    sudo docker rm -f $(sudo docker ps -aq --filter name=identity_) > /dev/null
fi
...
do
    ...
    echo "" > "logs/identity_${i}.txt"
done

# Build the docker image
sudo docker build -t identity . > logs/docker_build_output.txt 2> logs/docker_build_output.txt

# Start key value db (publish unique host ports for each container)
for (( i=0; i < $SERVER_QUANTITY; i++ ))
do
    CONTAINER_NAME="identity_${i}"
    HOST_PORT=$(($API_PORT + $i))
    ...
    CID=$(sudo docker run -d --name "${CONTAINER_NAME}" \
        ...
        -p "3014:${HTTP_PORT}/udp" \
        -p "3014:${HTTP_PORT}/tcp" \
        identity)
